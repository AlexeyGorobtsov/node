...
handlers._tokens.put = function (data, callback) {
    ...
    && String(data.payload.id).trim().length === 20 ? String(data.payload.id).trim() : false;
    const extend = typeof (data.payload.extend) === 'boolean' && data.payload.extend === true;
    if (id && extend) {
        // Lookup the token
        _data.read('tokens', id, function (err, tokenData) {
            if (!err && tokenData) {
                // Chek to the make sure the token isn't already expired
    ...
    if (id) {
        ...
        _data.read('tokens', id, function (err, data) {
            if (!err && data) {
                ...
                callback(400, {'Error': 'Missing required tokens'})
            }
        })
    } else {
        callback(400, {'Error': 'Missing required field'})
...
handlers._checks.post = function (data, callback) {
    ...
    && String(data.payload.url).trim().length > 0 ? String(data.payload.url).trim() : false;

    const method = typeof (data.payload.method) === 'string'
    && ['post', 'get', 'put', 'delete'].includes(data.payload.method) ? data.payload.method.trim() : false;
